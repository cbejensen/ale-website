<?php 
/*
 * This is the content of the Premium Equipment page,
 * in the Products & Services section of the site.
 *
 * This is a small, condensed catalog of our Automation
 * equipment that's used in our integrations and application
 * development. It's intended to give visitors a
 * general idea of the equipment we offer for these services,
 * and to direct them to our other, more specific ads/listings
 * where they can purchase these instruments a la carte.
 */
?>
<main class="premium-equipment-main">
	<h1>Premium Equipment</h1>
	<p>Our extensive inventory includes myriad instrumentation which enables
	automation of laboratory procedures. We develop custom
	integrated systems for specific applications using the equipment found herein, 
	and we're happy to provide these items individually, as well.</p>
	<p>Within each of the subsections you'll find links to the appropriate listings
	of our catalog where you can find more information on specific instruments.</p>
	<p>Alternatively, you can view our catalog in its entirety <a href="">here</a>.</p>
	<div class="premium-equipment-array">
		<div class="premium-equipment-category">
			<img src="" alt="">
			<h2>Tecan Liquid Handlers</h2>
		</div>
		<div class="premium-equipment-category">
			<img src="" alt="">
			<h2>Other Liquid Handlers</h2>
		</div>
		<div class="premium-equipment-category">
			<img src="" alt="">
			<h2>Analytical Systems</h2>
		</div>
		<div class="premium-equipment-category">
			<img src="" alt="">
			<h2>DNA Sequencers</h2>
		</div>
		<div class="premium-equipment-category">
			<img src="" alt="">
			<h2>Plate Readers</h2>
		</div>
		<div class="premium-equipment-category">
			<img src="" alt="">
			<h2>Plate Washers</h2>
		</div>
		<div class="premium-equipment-category">
			<img src="" alt="">
			<h2>Plate Sealers</h2>
		</div>
		<div class="premium-equipment-category">
			<img src="" alt="">
			<h2>Centrifuges</h2>
		</div>
	</div>
</main>
